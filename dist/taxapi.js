...
// src/v1/utils/merge.ts
var Merge = (headers, body) => {
...
// src/v1/utils/json.ts
var JsonHelper = class _JsonHelper {
  static deserializeAndFlatten(json) {
    const dict = {};
    const token = JSON.parse(json);
    _JsonHelper.fillDictionaryFromJToken(dict, token, "");
    return dict;
  }
      ...
      case "object":
        if (Array.isArray(token)) {
          for (let i = 0; i < token.length; i++) {
            _JsonHelper.fillDictionaryFromJToken(
              dict,
              token[i],
              _JsonHelper.join(prefix, `E${i}`)
            );
          }
        } else {
...
// src/types/version1.ts
var Version1;
((Version12) => {
  let Purpose;
  ((Purpose2) => {
    Purpose2["SEND_INVOICE_FAST"] = "fast-enqueue";
    Purpose2["SEND_INVOICE_NORMAL"] = "normal-enqueue";
    Purpose2["GET_TOKEN"] = "GET_TOKEN";
    Purpose2["GET_FISCAL_INFORMATION"] = "GET_FISCAL_INFORMATION";
    Purpose2["INQUIRY_BY_UID"] = "INQUIRY_BY_UID";
    Purpose2["INQUIRY_BY_REFERENCE_NUMBER"] = "INQUIRY_BY_REFERENCE_NUMBER";
    Purpose2["INQUIRY_BY_TIME"] = "INQUIRY_BY_TIME";
    Purpose2["INQUIRY_BY_TIME_RANGE"] = "INQUIRY_BY_TIME_RANGE";
    Purpose2["GET_SERVER_INFORMATION"] = "GET_SERVER_INFORMATION";
    Purpose2["GET_SERVICE_STUFF_LIST"] = "GET_SERVICE_STUFF_LIST";
    Purpose2["GET_ECONOMIC_CODE_INFORMATION"] = "GET_ECONOMIC_CODE_INFORMATION";
  })(Purpose = Version12.Purpose || (Version12.Purpose = {}));
  ;
})(Version1 || (Version1 = {}));

// src/v1/api/get-fiscalId.ts
async function getFiscalId(base, username, token, pem) {
  const url2 = base.replace("__PURPOSE__", Version1.Purpose.GET_FISCAL_INFORMATION);
  const headers = {
  ...
  const response = await axios_default.post(url2, data, config);
  return response.data;
}

// src/v1/api/get-server-information.ts
var getServerInformation = async (base) => {
  try {
    const url2 = base.replace("__PURPOSE__", Version1.Purpose.GET_SERVER_INFORMATION);
    const headers = {
      "requestTraceId": generateUUID(),
      "timestamp": Date.now().toString(),
    ...
    const res = await axios_default.post(url2, data, { headers });
    return res.data;
  } catch (err) {
    throw err;
  }
...
// src/v1/api/get-token.ts
async function getToken(base, username, pem) {
  const url2 = base.replace("__PURPOSE__", Version1.Purpose.GET_TOKEN);
  const headers = {
    "requestTraceId": generateUUID(),
    "timestamp": Date.now().toString()
  };
...
// src/v1/api/inquire-by-range.ts
async function inquireByRange(base, fiscalId, token, pem, startDate, endDate) {
  const url2 = base.replace("__PURPOSE__", Version1.Purpose.INQUIRY_BY_TIME_RANGE);
  const headers = {
    "requestTraceId": generateUUID(),
    "timestamp": Date.now().toString(),
    "Authorization": `Bearer ${token}`
...
// src/v1/api/inquire-by-uid.ts
async function inquireByUID(base, fiscalId, token, pem, UIDs) {
  const url2 = base.replace("__PURPOSE__", Version1.Purpose.INQUIRY_BY_UID);
  const headers = {
    "requestTraceId": generateUUID(),
    "timestamp": Date.now().toString(),
    "Authorization": `Bearer ${token}`
...
// src/v1/api/inquiry-economic-code.ts
var inquiryEconomicCode = async (base, economicCode) => {
  const url2 = base.replace("__PURPOSE__", Version1.Purpose.GET_ECONOMIC_CODE_INFORMATION);
  const headers = {
    "requestTraceId": generateUUID(),
    "timestamp": Date.now().toString()
  };
  ...
  return (await axios_default.post(url2, data, { headers })).data.result.data;
};

// src/v1/api/send-invoice.ts
var SendInvoice = async (type, invoices, token, publicKey, username, pem, retry) => {
  const url2 = `https://tp.tax.gov.ir/req/api/${type}/async/normal-enqueue`;
  const headers = {
    "requestTraceId": generateUUID(),
    "timestamp": Date.now().toString(),
    "Authorization": `Bearer ${token}`
...
// src/v1/constants/base.ts
var BASE_V1 = "https://tp.tax.gov.ir/req/api/__TYPE__/sync/__PURPOSE__";
var BASE_SANDBOX_V1 = "https://sandboxrc.tax.gov.ir/req/api/__TYPE__/sync/__PURPOSE__";
...
var PemReader = class {
  constructor(path_or_content) {
    ...
    else if ((0, import_fs.existsSync)(path_or_content))
      this.privateKey = (0, import_fs.readFileSync)(path_or_content, "utf-8");
    else
...
-----END PRIVATE KEY-----`;
      ...
      const privateKey = crypto3.createPrivateKey(this.privateKey);
  ...
  encryptSymmetricKeyWithRsaOaepSha256(symmetricKey, publicKey) {
    try {
      ...
      const rsaKey = crypto3.createPublicKey({
        key: publicKeyBuffer,
        format: "der",
        type: "spki"
      });
      const encryptedBuffer = crypto3.publicEncrypt(
        {
          key: rsaKey,
...
var TaxApiV1 = class {
  constructor(pemFile, fiscalId, economicCode, type = "self-tsp", sandbox) {
    ...
    this.type = type;
    this.getToken = () => getToken(this.base, this.fiscalId, this.pem);
    this.getServerInformation = () => getServerInformation(this.base);
    this.inquiryEconomicCode = () => inquiryEconomicCode(this.base, this.economicCode);
    this.getFiscalId = (token) => getFiscalId(this.base, this.fiscalId, token, this.pem);
    this.inquireByRange = (token, start, end) => inquireByRange(this.base, this.fiscalId, token, this.pem, start, end);
    this.inquireByUID = (token, UIDs) => inquireByUID(this.base, this.fiscalId, token, this.pem, UIDs);
    this.sendInvoiceNormal = (props) => SendInvoice(
      this.type,
      props.invoices,
      props.token,
      props.publicKey,
...
mime-types/index.js:
   ...
   *)
*/
//# sourceMappingURL=taxapi.js.map
